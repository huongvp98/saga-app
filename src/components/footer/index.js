...
function Index(props) {
  const keyInit = Number(window.location.pathname[window.location.pathname.length - 1])

  const getColor = (keyInit) => {
    switch (keyInit) {
      case 1: return "#B71F35"
      case 2: return "#193D6B"
      case 3: return "#2C9595"
      case 4: return "#5EA661"
      case 5: return "#F5A623"
      default: break;
    }
  }
  const [keyActive, setKeyActive] = useState({ key: keyInit, background: getColor(keyInit) })
  console.log(keyActive)
  const history = useHistory()
  const handleClick = (value) => {
    setKeyActive(value)
    history.push(`/component/step${value.key}`)
  }
  return (
    <Footer>
